<template>
  <div ref="wrapper" class="gulu-table-wrapper">
    <div class="gulu-table-scroll" :style="{height, overflow: 'auto'}" ref="tableWrapper">
      <table ref="table" class="gulu-table" :class="{bordered, compact, striped}">
        <thead>
          <tr>
          ...
          <template v-for="(item, index) in dataSource">
            <tr :key="item.id">
              <td v-if="expendField" :style="{width: '50px'}" class="gulu-table-center">
                <g-icon @click="expendItem(item.id)" name="right" class="gulu-table-expend-icon"></g-icon>
              </td>
            ...
            <tr v-if="inExpendedIds(item.id)" :key="`${item.id}-expend`">
              <td :colspan="columns.length + expendedCellColSpan">{{item[expendField] || '空'}}</td>
            </tr>
          </template>
        </tbody>
      </table>
